...
uint64_t getRealOffset(uint64_t offset){
    // IL2CPP dump RVAs include 0x100000000 base but UnityFramework preferred base is 0x0
    // Subtract the main binary base to get the true file offset within UnityFramework
    MemoryFileInfo info = getMemoryFileInfo("UnityFramework");
    if (info.address == 0) {
        // Fallback to main binary if UnityFramework not found
        info = getBaseInfo();
        if (info.address == 0) return 0;
        return info.address + offset;
    }
    uint64_t realOffset = offset;
    if (realOffset >= 0x100000000ULL) {
        realOffset -= 0x100000000ULL;
    }
    return info.address + realOffset;
}
